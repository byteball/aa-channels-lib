...
async function getChannelsForPeer(peer, asset, handle){
	...
	if (peer.match(/^([\w\/+]+)@([\w.:\/-]+)#([\w\/+-]+)$/)){
		...
		const rows = await appDB.query("SELECT aa_address FROM channels WHERE peer_address=? AND asset=? ORDER BY creation_date ASC",[peer, asset]);
		if (rows.length === 0)
...
function setMinimumTimeToWaitForUnconfirmedPaymentByAsset(asset, time){
	...
	if (asset != 'base' &&	!validationUtils.isValidBase64(asset, 44))
		return false;
	initializeUnconfirmedAmountsLimitsByAssetOrChannel(asset);
	conf.unconfirmedAmountsLimitsByAssetOrChannel[asset].minimum_time_in_seconds = time;
	return true;
}
